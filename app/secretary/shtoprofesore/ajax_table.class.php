...
class ajax_table {
  ...
  function getRecords(){
      ...
      $this->res = mysqli_query($this->conn,"select * from profesoret  where D_id='$dep_id' and Dg_id='$dega_id' ORDER BY P_id DESC");
  ...
  function save($data){
	...
	if(count($data)){
		...
		mysqli_query($this->conn,"insert into profesoret (".$vlerat.",Type,Username,password,D_id,Dg_id) values ('$values','profesor','$username','$password','$dep_id','$dega_id')");
  ...
  function delete_record($id){
	 if($id){
		mysqli_query($this->conn,"delete from profesoret where P_id = $id limit 1");
  ...
  function update_record($data){
	if(count($data)){
		...
		$str = substr($str,0,-1);
		$sql = "update profesoret set $str where P_id = $id limit 1";
  ...
  function update_column($data){
	if(count($data)){
		$id = $data['rid'];
		unset($data['rid']);
		$sql = "update profesoret set ".key($data)."='".$data[key($data)]."' where P_id = $id limit 1";
		$res = mysqli_query($this->conn,$sql);
		if(mysqli_affected_rows($this->conn)) return $id;
		return 0;
