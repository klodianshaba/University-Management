              ...
              <div class="menu-container" style="margin-left:-10px;margin-top:0px">
                  <ul>
                      ...
                      <li class="tabs"><a href="#">Notat</a> <span class="plus" id="plus6">+</span>
                          <ul class="submenu" value="none" id="sub3">
                              <li><a href=""><img src="../img/sign.png" class="circle-icon-sm">Shiko Notat</a></li>
                          </ul>
                      </li>
...
<div class="title-bar" style="width: 100%;
    ...
    color: white;
    padding: 10px;"><div id="" style="padding-top: 10px;">Notat & Informacione</div></div>

    <div id="etera" class="table--class" style="width: calc(100% - 20px); ">
    ...
    <table>
        ...
        <?php 
         ...
         ?>
        <tr>
          <td style=" width: 100%;text-align:center; color:#fff;padding:10px;font-size:12px;     background-color: #29B6F6;" >
          Viti i pare
          </td>
        </tr>
        ...
        <?php 
          ...
          $query = mysqli_query($conn,"SELECT L_id,Emri,Kredite,L_P_id from lendet where Viti='1' AND Dg_id='$Dg_id'");
          $sum_kred1 = 0;$kaluar1=0;$ngeles1=0;
          $sum_nota1 = 0;$lendet1=0;$nota_vendosura1 =0;
          while ($row = mysqli_fetch_assoc($query)) {
            ...
            <tr>
              ...
              <td><?php 
                $L_id = $row['L_id'];
                $nota = mysqli_query($conn,"SELECT Nota from notat where L_id='$L_id' AND ID='$id_st'");
                $nota = mysqli_fetch_assoc($nota);
                $nota = $nota['Nota'];
              if (empty($nota)) {
                echo "<center>",$row['Kredite'],"</center>";
              }else if($nota>4){
                echo "<center>",$row['Kredite'],"</center>";
                $sum_kred1 += $row['Kredite'];
              }else{
              ...
              if(empty($nota)){
                echo "<center>Pa Filluar</center>";
              }else if ($nota>4) {
                echo "<center>Kaluar</center>";
                $kaluar1++;
                $sum_nota1 +=$nota;
                $nota_vendosura1++;
              }else{
                echo "<center>Ngeles</center>";
                $ngeles1++;
              }

               ?></td>

               <td><?php if (empty($nota)) {
                 echo "<br>";
               }else{
                echo "<center>",$nota,"</center>";
                }; ?></td>

            </tr>
          <?php }
          $ka = mysqli_num_rows($query);
          if($ka>0){ ?>
          <tr>
            <td><center>TOTALI: <?php echo $lendet1;?></center></td>        
            <td><center>TOTALI: <?php echo $lendet1;?></center></td>        
            <td><center>TOTALI: <?php echo "Kreditet nga lendet kaluese: ",$sum_kred1;?></center></td>        
            <td><center>TOTALI: <?php echo "Lende te kaluara: ",$kaluar1," nga ",$lendet1;?></center></td>        
            <td><center>MESATARE: <?php 
                                      if (!$nota_vendosura1) {
                                        echo "";
                                      }else{
                                        echo $sum_nota1/$nota_vendosura1; ?>       
                                      <?php } ?>  </center></td> 
         ...
         ?>
          ...
          $query = mysqli_query($conn,"SELECT L_id,Emri,Kredite,L_P_id from lendet where Viti='2' AND Dg_id='$Dg_id'");
          $sum_kred2 = 0;$kaluar2=0;$ngeles2=0;
          $sum_nota2 = 0;$lendet2=0;$nota_vendosura2=0;
          while ($row = mysqli_fetch_assoc($query)) {
             ?>
            <tr>
          ...
          if($ka>0){ ?>
          <tr>
            <td><center>TOTALI: <?php echo $lendet2;?></center></td>        
            <td><center>TOTALI: <?php echo $lendet2;?></center></td>        
            <td><center>TOTALI: <?php echo "Kreditet nga lendet kaluese: ",$sum_kred2;?></center></td>        
            <td><center>TOTALI: <?php echo "Lende te kaluara: ",$kaluar2," nga ",$lendet2;?></center></td>        
            <td><center>MESATARE: <?php 
                                      if (!$nota_vendosura2) {
                                        echo "";
                                      }else{
                                        echo $sum_nota2/$nota_vendosura2; ?>       
                                      <?php } ?>  </center></td>     
         ...
         ?>
          ...
          Viti i Trete
           ...
           // $query = mysqli_query($conn,"SELECT L.L_id,L.Emri,L.kredite,L.L_P_id,N.Nota  from lendet L join notat N on L.L_id = N.L_id  where Viti='$Viti' AND Dg_id='$Dg_id'");
          $query = mysqli_query($conn,"SELECT L_id,Emri,Kredite,L_P_id from lendet where Viti='3' AND Dg_id='$Dg_id'");
          $sum_kred3 = 0;$kaluar3=0;$ngeles3=0;
          $sum_nota3 = 0;$lendet3=0;$nota_vendosura3=0;
          while ($row = mysqli_fetch_assoc($query)) {
             ?>
            <tr>
          ...
          if($ka>0){ ?>
          <tr>
            <td><center>TOTALI: <?php echo $lendet3;?></center></td>        
            <td><center>TOTALI: <?php echo $lendet3;?></center></td>        
            <td><center>TOTALI: <?php echo "Kreditet nga lendet kaluese: ",$sum_kred3;?></center></td>        
            <td><center>TOTALI: <?php echo "Lende te kaluara: ",$kaluar3," nga ",$lendet3;?></center></td>        
            <td><center>MESATARE: <?php 
                                      if (!$nota_vendosura3) {
                                        echo "";
                                      }else{
                                        echo $sum_nota3/$nota_vendosura3; ?>       
                                      <?php } ?>  </center></td>         
         ...
         <tr>
            <td style="padding:10px;font-size:12px;background-color: #354052;color:#fff"><center>TOTALI: <?php echo $lendet3+$lendet2+$lendet1;?></center></td>        
            <td style="padding:10px;font-size:12px;background-color: #354052;color:#fff"><center>TOTALI: <?php echo $lendet3+$lendet2+$lendet1;?></center></td>        
            <td style="padding:10px;font-size:12px;background-color: #354052;color:#fff"><center>TOTALI: <?php echo "Kreditet nga lendet kaluese: ",$sum_kred3+$sum_kred2+$sum_kred1;?></center></td>        
            <td style="padding:10px;font-size:12px;background-color: #354052;color:#fff"><center>TOTALI: <?php echo "Lende te kaluara: ",$kaluar3+$kaluar2+$kaluar1," nga ",$lendet3+$lendet2+$lendet1;?></center></td>        
            <td style="padding:10px;font-size:12px;background-color: #354052;color:#fff"><center>MESATARE:<?php 
                                      if (!$nota_vendosura1 && !$nota_vendosura2 && !$nota_vendosura3) {
                                        echo "";
                                      }else{
                                        echo ($sum_nota3+$sum_nota2+$sum_nota1)/($nota_vendosura1+$nota_vendosura2+$nota_vendosura3); ?>       
                                      <?php } ?>  </center></td>         
